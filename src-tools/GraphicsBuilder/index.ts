...
let inPathPacked = path.join(basePath, 'src-deploy', 'public', dir, 'images')  + '\\'
let outPathPacked = path.join(basePath, 'src-deploy', 'public', dir, 'images-packed')  + '\\'
...
const _throttled_runPacker = _.throttle(runPacker, 1000, { leading: false, trailing: true })
...
function _pack(inPath, outPath, outJson, outPng) {
  ...
  let scale = 1
  let args = `--data ${outPath + outJson} --format json --sheet ${outPath + outPng} --force-squared --scale ${scale} --scale-mode fast --width 512 --height 512 --border-padding 2 --shape-padding 2 --padding 2 --trim-mode Trim --trim-sprite-names --disable-rotation ${inPath}`
